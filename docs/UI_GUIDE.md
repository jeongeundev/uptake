...
> 이 문서는 phase 1 이식 UI와 phase 2 카탈로그 저작 UI의 확정 계약이다. INSTANTIATE·VERIFY·승인·적용 및 EXTRACT·ABSTRACT·승인·등재 계약을 사용자에게 정직하게 노출한다.
...
저작 wizard는 다음 순서로 진행한다.

1. 사용자가 패턴 식별 정보, 분류축, 의도와 소스 저장소를 입력한다.
2. 서버가 반환한 roles, binding points, provenance, corroboration 계산, 강등된 role, 버려진 후보, target stack 관찰 사실, oracle 자기검증과 tradeoffs를 검토한다.
3. 후보를 편집하지 않고 명시적으로 승인하거나 거부한다. 승인은 서버 저장소의 상태 전이이며 클라이언트 boolean이 권한을 대신하지 않는다.
4. 서버 승인 뒤에만 등재 버튼이 활성화된다.
5. 등재 성공 시 기록 경로를 표시하고, 실패 시 원문 사유를 표시한다. `pattern-exists`에는 기존 패턴이 변경되지 않았음을 함께 명시한다.

`independenceGroup`과 `isTargetStack`은 큐레이터인 사용자의 판정이다. 앱은 두 필드를 자동으로 채우거나 추천값을 미리 넣지 않는다. `targetStackFacts`는 “vitest가 관찰됨/관찰되지 않음”과 근거 경로만 표시하며 타깃 스택 판정 문구로 바꾸지 않는다.

corroboration은 사용자가 입력한 `independenceGroup`의 역할별 목록과 distinct 개수를 표시한다. 이 계산 결과가 앱의 판정이 아니라 사용자가 입력한 그룹을 센 결과임을 화면에 명시한다.

강등된 role과 버려진 후보는 개수가 0이 아닐 때 반드시 각 항목과 사유를 표시한다. 특히 `provenance-unresolved`를 성공 요약 뒤에 숨기지 않는다.

### 저작 자기검증 상태

| 상태 | 사용자 문구 | 색/행동 |
|---|---|---|
| `passed` | 양성 green과 음성 red가 확인되었습니다. | green, 승인 가능 |
| `failed` / `self-verify-failed` | 자기검증에 실패해 승인과 등재가 차단되었습니다. 상세 사유를 함께 표시합니다. | red, 승인·등재 차단 |
| `skipped` | descriptive 패턴이므로 자기검증을 건너뛰었습니다. | 중립, 승인 가능 |

`negative-not-caught`는 게이트가 위반 상태에서도 green이었기 때문에 실패한 상태다. raw green을 성공색으로 표시하지 않고 red로 표시하며 승인 경로를 열지 않는다.
