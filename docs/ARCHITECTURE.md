...
- **승인은 클라이언트 상태가 아니다.** 위의 "승인 여부"는 화면 조작일 뿐이고, 적용 API는 **클라이언트가 보낸 boolean을 신뢰하지 않는다.** 승인 레코드는 서버측에 남으며 최소한 타깃 경로 · `patternId` · 산출물 해시 · 타깃 base 해시 · 동결된 argv에 결속된다. 그래야 UI를 우회한 직접 API 호출로 미승인 적용이 일어나지 않는다(AC-10). 세션 저장 방식(메모리/파일)과 수명은 apply 구현 시 정한다.
...
| 항목 | 확정 시점 |
|---|---|
| 결합점 탐지 결과 계약 — 값·근거 경로의 타입, 복수 후보, 사용자 입력값과 자동 탐지값의 구분, `kind` 4종의 탐지 규칙 | 결합점 탐지 구현 (AC-4) |
...
| 승인의 서버측 결속 — 세션/토큰 모델, 수명, 새로고침·다중 탭, API 직접 호출 차단 | apply 구현 (AC-10) |
